...
import {getPublicKey, getTransactionId, signTxIn, Transaction, TxIn, TxOut, UnspentTxOut, hash160PublicKey} from './transaction';
...
const createTransaction = (receiverAddress: string, amount: number, privateKey: string,
    ...
    const myAddress: string = hash160PublicKey(getPublicKey(privateKey));
    ...
    console.log("passed filter unspentOutputs");
    ...
    console.log("passed unsigned");

    const tx: Transaction = new Transaction();
    tx.txIns = unsignedTxIns;
    tx.txOuts = createTxOuts(receiverAddress, myAddress, amount, leftOverAmount);
    console.log("passed created TxOuts");
    ...
    tx.txIns = tx.txIns.map((txIn: TxIn, index: number) => {
        // here !
        txIn.scriptSig = signTxIn(tx, index, privateKey, unspentTxOuts);
        return txIn;
    });
