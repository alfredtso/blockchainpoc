...
import {hash160PublicKey} from './transaction';
...
const initHttpServer = (myHttpPort: number) => {
    ...
    app.get('/btaddress', (req, res) => {
        const address: string = getPublicFromWallet();
        const btaddress: string = hash160PublicKey(address);
        res.send({'address': btaddress});
    });
    ...
    app.post('/sendTransaction', (req, res) => {
        try {
            ...
            if (address === undefined || amount === undefined) {
                throw Error('this invalid address or amount');
            }
