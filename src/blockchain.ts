...
const genesisTransaction = {
    'txIns': [{'scriptSig': [], 'txOutId': '', 'txOutIndex': 0}],
    'txOuts': [{
...
// in seconds
const BLOCK_GENERATION_INTERVAL: number = 0.2;
// in blocks
...
const generateNextBlock = () => {
    const coinbaseTx: Transaction = getCoinbaseTransaction(hash160PublicKey(getPublicFromWallet()), getLatestBlock().index + 1);
...
const getAccountBalance = (): number => {
    return getBalance(hash160PublicKey(getPublicFromWallet()), getUnspentTxOuts());
};

const sendTransaction = (address: string, amount: number): Transaction => {
    const tx: Transaction = createTransaction(address, amount, getPrivateFromWallet(), getUnspentTxOuts(), getTransactionPool());
    console.log('going to add to transaction pool');
...
// Here we will be using the JS 'crypto-js' library for SHA256
...
const hashMatchesDifficulty = (hash: string, difficulty: number): boolean => {
    ...
    if (difficulty < 0) {
        const requiredPrefix: string = '0'.repeat(0);
...
const hash160PublicKey = (aPublicKey: string): string => {
    ...
    console.log('Print the key: ' + JSON.stringify(value));
    return value;
};
